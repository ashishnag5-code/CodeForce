...
export default class GenericCardPaginationDisplayCredit extends NavigationMixin(LightningElement) {
    @api recordId;
    @api objectList;
    @api cardTitle;
    @api enableSelection;
    ...
    @api breResponseId;
    @api showUwAuthority=false;
    @api loanSchemeCode;
    @api schemeAddDetails;
    @api deviationsDetails
    @api getSchemeDetails=false;
    ...
    displayDetails(event) {
        ...
        for(var i = 0; i < this.schemeAddDetails.length; i++){ 
            ...
            if(this.schemeAddDetails[i].key != undefined && clickedScheme == this.schemeAddDetails[i].key.trim()){
                ...
                let subjectList = this.schemeAddvalues["subjectivityMessages"];

                if (subjectList != undefined) {
                    let tempER = [];
                    for (var j = 0; j < subjectList.length; j++) {
                        var tempArr={};
                        tempArr['Subjectivity'] = subjectList[j]['message'];
                        tempArr['Owner'] = subjectList[j]['type'];
                        tempER.push(tempArr);
                    }
                    if(tempER.length !=0){
   ...
   CheckRemarksValue() {
    ...
    connectedCallback() {
     ...
     if(this.showUwAuthority==false && this.schemeAddDetails != undefined && this.schemeAddDetails.length > 0){
     this.schemeAddDetails.forEach(item => {
        if (item.key === 'uwAuthority') {
            this.uwAuthority = item.value;
            this.showUwAuthority=true;
        }
    });
}
    ...
    setUrlPath() {
        ...
        else if (this.cardTitle == "Eligible Scheme") {
    ...
    handleObjDataList() {
        ...
        for (const key in this.objectList[this.counter]) {
            ...
            if (key == 'deviationsDetails') {
                ...
                let tempER = [];
                if((this.objectList[0]['deviationsDetails'].length != 0) && (this.objectList.length>0) && (this.objectList[0]['deviationsDetails'] != 'undefined') ){
