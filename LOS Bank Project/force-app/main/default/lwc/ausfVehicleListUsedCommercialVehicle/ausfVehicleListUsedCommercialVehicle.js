...
import getDepreciatedMaster from '@salesforce/apex/AUSFCommercialVehicleController.getDepreciatedMaster';
...
import { getLoanType, reduceErrors, getLeastAmount, OTHER_FUNDING_ITEMS_MAPPINGS, validateLoanFunding } from 'c/lwcutilities';
import {
    subscribe,
    unsubscribe,
    APPLICATION_SCOPE,
...
export default class AusfVehicleListUsedCommercialVehicle extends NavigationMixin(LightningElement) {
    @api recordId;
    @api spinnerImage;
    ...
    */
    ...
    fetchState(loanApplRecordId) {
		...
		.then(async data => {
            if (data) {
                ...
                console.log('Body Type ==> ', [...data.collateralNames.Body_Type__c]);
                            ...
                            for (let i = 0; i < dataCol.length; i++) {
                                ...
                                //if(dataCol[i].Certified_Value__c){
                                    ...
                                    if(certifiedVal>0 && certifiedVal<=constVal){
                                        dataCol[i].isDisableValuation = !dataCol.Is_Collateral_Updated_Post_Creation__c;//this.collateralUpdates[dataCol[i].Id];;
    ...
    handleCopyToCollateral(event){
        ...
        if(!this.newVehicleRecord.Collateral_Name__c){
            this.newVehicleRecord.Collateral_Name__c = this.loanApplicationRecord.Collateral_Type__c;
        }
    ...
    handleGetDepValue(){
		...
		.then(data => {
            if (data) {
                ...
                this.newVehicleRecord = { ...this.newVehicleRecord, Grid_Depreciated_Value__c : data['X'+this.manufactureYear+'__c'] ?? 'NA' };
                this.isLoaded = false;
            }
		})
		.catch(error => {
		...
		.then(result => {
            ...
            result.forEach(input => {
                if(this.template.querySelector('[data-id="'+input+'"]') !== null){
                    [...this.template.querySelectorAll('[data-id="'+input+'"]')].forEach( item => item.classList.remove('slds-hide'));
                }
            });
            this.isLoaded = false;
		})
    ...
    async getDepreciatedMaster(){
        this.isLoaded = true;
        const collateral = { ...this.newVehicleRecord },
            loanApp = { ...this.loanApplicationRecord };
        const depreciatedMaster = await getDepreciatedMaster({ collateral, loanApp })
            .catch(error => {
                console.log('error is '+JSON.stringify(error));
                this.isLoaded = false;
            });
        console.log({ depreciatedMaster });
        ...
        restricAccess({
            ...
            .then(data => {
                ...
                if(!isValid || !isYearValid){
                    ...
                    this.showToast( 'Funding items if taken should be greater than 0' );
                    return;
                ...
                const collateral = { ...this.newVehicleRecord };
                const valuationAmt = +collateral.Valuation_Amount__c || 0,
                    otherFunding = this.otherFundingTotal,
                    apportionedLoan = +collateral.Apportioned_Loan_Amount__c || 0;
                if( otherFunding > valuationAmt ){
                    this.showToast( 'Funding cannot be more than Valuation Amount','error' );
                    return;
                }
                if( valuationAmt && apportionedLoan > valuationAmt ){
    ...
    async upsertVehicleInfo() {
        ...
        currentObj.Collateral_Name__c = this.loanApplicationRecord.Collateral_Type__c;
        //this.newVehicleRecord = currentObj;
    ...
    calculateLoanAmtGridDifference(){
        ...
        let Amt = this.newVehicleRecord.Loan__r?.Total_Loan_Amount__c ?? this.loanApplicationRecord?.Total_Loan_Amount__c;
    ...
    get ltvOfferedOnVehicleFinalCost(){
        ...
        // let loanAmount = this.newVehicleRecord.Apportioned_Loan_Amount__c ? this.newVehicleRecord.Apportioned_Loan_Amount__c : 0;
        const totalLoanAmount = this.totalLoanAmount;//this.newVehicleRecord.Loan__r?.Total_Loan_Amount__c ?? this.loanApplicationRecord?.Total_Loan_Amount__c;
    ...
    get finalPrice(){
        ...
        const certifiedValueNum = +certifiedValue?.replace(/,/g, ''), valuationAmountNum = +valuationAmount?.replace(/,/g, ''), depreciatedValueNum = +depreciatedValue?.replace(/,/g, ''), ibbValueNum = +ibbValue?.replace(/,/g, '');

        const applicableCost = getLeastAmount( vehicleCost, [ depreciatedValueNum || ibbValueNum, certifiedValueNum || valuationAmountNum ] ) || 0;
    ...
    get vehicleCost(){
        ...
        const valuationAmountNum = +valuationAmount?.replace(/,/g, ''), certifiedValueNum = +certifiedValue?.replace(/,/g, ''), depreciatedValueNum = +depreciatedValue?.replace(/,/g, ''), ibbValueNum = +ibbValue?.replace(/,/g, '');
        ...
        let isValid = inputFields.reduce((isValidSoFar, inputField) => {
            if (visibledFieldList.includes(inputField.name)) {
                ...
                if(!inputField.value){
                    ...
                    fieldErrors[ inputField.name ] = true;
                    ...
                    fieldErrors[ inputField.name ] = false;
                    isFieldValid = true;
    ...
    getPicklistOptions(makeValue,modelValue,collateralNameValue,fieldNameValue){
        ...
        getPickListValues({ make:makeValue,model:modelValue ,collateralName:collateralNameValue, fieldName:fieldNameValue, collateralType: this.loanApplicationRecord.Collateral_Type__c ?? '', strScreen: 'Used',implementType:'',loanProduct : this.loanApplicationRecord.Product__c })
		.then(data => {
            if (data) {
                console.log('data is>>'+JSON.stringify(data))
    ...
    getValuationSettingsBasedOnScheme = (applicants, schemeName) => applicants.map(applicant => ({ ...applicant, isDisableValuation: schemeName === branchTopup || !applicant.Is_Collateral_Updated_Post_Creation__c }));
