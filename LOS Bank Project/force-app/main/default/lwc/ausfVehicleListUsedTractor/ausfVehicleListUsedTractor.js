...
import getDepricatedTractorMasterRecord from '@salesforce/apex/AUSFVehicleController.getDepricatedTractorMasterRecord';
...
import S4GridMinDiffPercentage from '@salesforce/label/c.S4GridMinDiffPercentage';
import S4GridMaxDiffPercentage from '@salesforce/label/c.S4GridMaxDiffPercentage';
import S5GridMaxDiffPercentage from '@salesforce/label/c.S5GridMaxDiffPercentage';
...
export default class AusfVehicleListUsedTractor extends NavigationMixin(LightningElement) {
    @api recordId;
    ...
    */
    ...
    get productSubTypeOptions() {
        return [
            ...
            { label: 'Tractor + Loader & Dozer', value: 'Tractor + Loader & Dozer' },
    ...
    get ltvOffered(){
        const loanAmt =parseFloat(this.loanApplicationRecord.Loan_Amount__c); 
        const cost = parseFloat(this.vehicleCost);

        let ltv  = cost!=0 ? loanAmt/cost != 0 ? loanAmt/cost : 0 :0;
         if(ltv!=0){
            ltv = ltv *100;
        }
		...
		.then(async data => {
            if (data) {
                ...
                if(data.result && Object.keys(data.result).length){
                    ...
                    this.errorMessage = 'Vahaan API Failure.';
                    this.desableField.Make__c = false;
    ...
    handleSubmitForm(){
        restricAccess({
            ...
            .then(data => {
                ...
                if(!this.newVehicleRecord.Grid_Depreciated_Value__c){
                    this.showToast('Please Fetch Depreciated Value ','error');
                    return;
                }
                ...
                if(this.isCertifiedRequired && !this.newVehicleRecord.Certified_Value__c){
                    this.showToast('Please Add Certified Value','error');
                    return;
                }
                let currentObj = Object.assign({}, this.newVehicleRecord);
    ...
    async upsertVehicleInfo() {
        this.isLoaded = true;
        // this.finalPrice();
        this.ltvOfferedOnVehicleFinalCost();
        ...
        if(approvedLTV == 0 && valuationAmt!=0){
             this.newVehicleRecord.Approved_LTV__c = valuationAmt;
        }else{
            this.newVehicleRecord.Approved_LTV__c = approvedLTV < valuationAmt ? approvedLTV.toString() : valuationAmt.toString();
        }
        ...
        if (currentObj) {
            ...
            this.isloading = true;
            console.log('this.aplicantRecord',JSON.stringify(currentObj));
    ...
    get isLTVInvalid(){
        ...
        return !!( +this.newVehicleRecord.Apportioned_Loan_Amount__c && (this.ltvOffered > 100));
    }

     ltvOfferedOnVehicleFinalCost(){
        let currentObj = Object.assign({}, this.newVehicleRecord);
    ...
    get loanEligibilty(){
        let currentObj = Object.assign({}, this.newVehicleRecord);
        let loanAmount = this.newVehicleRecord.Apportioned_Loan_Amount__c ? this.newVehicleRecord.Apportioned_Loan_Amount__c : 0;
        let approvedLtvAmount = currentObj.Approved_LTV__c?parseFloat(currentObj.Approved_LTV__c):0;
        let vehicleCost = currentObj.Vehicle_Cost__c?parseFloat(currentObj.Vehicle_Cost__c):0;
        let finalValue=0;
        //parseInt Converts a string to an integer.
          ...
          /*  if(parseInt(approvedLtvAmount,10) && parseInt(loanAmount,10)!==0){
            ...
            finalValue = (approvedLtvAmount *  vehicleCost)/100;
        return finalValue.toFixed(2);
    }
     ...
     */
     finalPrice(){
        ...
        const certifiedValueNum = +certifiedValue?.replace(/,/g, ''), valuationAmountNum = +valuationAmount?.replace(/,/g, ''), ibbValueNum = +ibbValue?.replace(/,/g, ''), depreciatedValueNum = +depreciatedValue?.replace(/,/g, '');

        const certifiedAmount = certifiedValueNum || valuationAmountNum;
        const applicableCost = getLeastAmount( vehicleCost, [ depreciatedValueNum, certifiedAmount ] ) || 0;
        //parseInt Converts a string to an integer.
    ...
    get vehicleCost(){
     ...
     handlevehicleCost(){
        ...
        let depreciatedVal = this.newVehicleRecord.Grid_Depreciated_Value__c!=null && this.newVehicleRecord.Grid_Depreciated_Value__c!='' && this.newVehicleRecord.Grid_Depreciated_Value__c!=NaN  && this.newVehicleRecord.Grid_Depreciated_Value__c!=undefined ? parseFloat(this.newVehicleRecord.Grid_Depreciated_Value__c):0;
        let valuationAmt = this.newVehicleRecord.Valuation_Amount__c!=null && this.newVehicleRecord.Valuation_Amount__c!='' && this.newVehicleRecord.Valuation_Amount__c!=NaN  && this.newVehicleRecord.Valuation_Amount__c!=undefined? parseFloat(this.newVehicleRecord.Valuation_Amount__c) :0;
        this.newVehicleRecord.Vehicle_Cost__c =  depreciatedVal!=undefined && depreciatedVal!=0 ? depreciatedVal :0;
        if(depreciatedVal == 0 && valuationAmt!=0){
             this.newVehicleRecord.Vehicle_Cost__c = valuationAmt;
        }else if( valuationAmt == 0 && depreciatedVal!=0){
             this.newVehicleRecord.Vehicle_Cost__c = depreciatedVal;
        }else{
            this.newVehicleRecord.Vehicle_Cost__c = depreciatedVal < valuationAmt ? depreciatedVal : valuationAmt;
        }
    ...
    isInputValid() {
        ...
        if(this.isLTVInvalid){
            this.showToast( 'LTV Offered can\'t exceed 100%', 'error' );
            isValid = false;
        } else if(this.totalPrincipalOutstanding && isValid && ((this.totalPrincipalOutstanding + +loanAmount) > vehicleFinalCostNum)){
    ...
    handleVaiantValueChange(event){
        ...
        this.getHpValue(event.target.value);
    ...
    getDLPFMPMaster(){
        ...
        Promise.all([
            ...
            getDepricatedTractorMasterRecord({ loanApp: this.loanApplicationRecord, mapOfColl: JSON.stringify(this.newVehicleRecord)}),
            ...
            if(values[1] != undefined &&  !this.newVehicleRecord.Approved_LTV__c){
                this.dipreciatedMasterRecord = values[1];
                this.newVehicleRecord.LTV_Record__c = JSON.stringify(values[1]);
                 if(this.applicantFinancialsRecord && this.newVehicleRecord.Original_Vehicle_Usage__c == 'Agri'){
                    if(this.applicantFinancialsRecord.Size_of_Agri_Land__c == 0){
                        this.newVehicleRecord.Approved_LTV__c = this.dipreciatedMasterRecord.Agri_0_Land__c;
                    }
                    if(this.applicantFinancialsRecord.Size_of_Agri_Land__c < 3){
                        this.newVehicleRecord.Approved_LTV__c = this.dipreciatedMasterRecord.Agri_Less_than_3_Land__c;
                    }
                    if(this.applicantFinancialsRecord.Size_of_Agri_Land__c >= 3){
                        this.newVehicleRecord.Approved_LTV__c = this.dipreciatedMasterRecord.Agri_More_than_3_Land__c;
                    }
                }
                if(this.applicantFinancialsRecord && this.newVehicleRecord.Original_Vehicle_Usage__c == 'Commercial'){
                    if(this.applicantFinancialsRecord.Applicant__r.Loan__r.Customer_Grade__c == 'FTU'){
                        this.newVehicleRecord.Approved_LTV__c = this.dipreciatedMasterRecord.Commercial_FTU__c;
                    }
                    if(this.applicantFinancialsRecord.Applicant__r.Loan__r.Customer_Grade__c == 'FTB'){
                        this.newVehicleRecord.Approved_LTV__c = this.dipreciatedMasterRecord.Commercial_FTB__c;
                    }
                    if(this.applicantFinancialsRecord.Applicant__r.Loan__r.Customer_Grade__c == 'Transporter'){
                        this.newVehicleRecord.Approved_LTV__c = this.dipreciatedMasterRecord.Commercial_Transporter__c;
                    }
                    if(this.applicantFinancialsRecord.Applicant__r.Loan__r.Customer_Grade__c == 'Captive'){
                        this.newVehicleRecord.Approved_LTV__c = this.dipreciatedMasterRecord.Commercial_Captive__c;
                    }
                }
            }
           
            this.newVehicleRecord = {...this.newVehicleRecord};
            // this.handlevehicleCost(); // same logic already there in vehicle cost getter
            this.isLoaded = false;
        })
        .catch(error => {
            console.log('error is ' + JSON.stringify(error));
    ...
    wiredValues({ error, data }) {
        if (data) {
            ...
            const filteredEquipment = options.filter(item => item.value!='BS6');
    ...
    calculateLoanAmtGridDifference(){
        ...
        if( differencePercentage >= S4GridMinDiffPercentage && differencePercentage <=S4GridMaxDiffPercentage ){
