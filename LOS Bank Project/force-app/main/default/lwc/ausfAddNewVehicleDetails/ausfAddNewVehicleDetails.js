...
const DELAY = 500;  
export default class AusfAddNewVehicleDetails extends NavigationMixin(LightningElement) {

    @api records; 
    @api recordId;                  //stores the data coming from parent component
    @api configurableMetaDataTable='Applicant_Data';  //Custom Metadata Api Name coming from parent component
    ...
    @api title;        
    @api treeModel;         
    @api manufacturerOptions;     //Title for the component, will get while placing in app builder
   ...
   connectedCallback(){
    console.log('data is>> '+JSON.stringify(this.records));
    console.log('picklist values2>> '+this.treeModel)
   // console.log(`screen width from parent ${this.screenWidth}`)
    ...
    getCustomMetadataColumnss({metadataName : this.configurableMetaDataTable}).then(result =>{
        ...
        console.log(`Fields_Data_Tables__r Field --> ${result[0].Fields_Data_Tables__r}`)
        ...
        console.log('columns are >>'+JSON.stringify(this.columnsData))
  ...
  setScreenSizes(){
      if(this.screenWidth === 'SMALL'){
          ...
          this.largeDeviceSize = 12
          this.mediumDeviceSize = 12
          this.filterMediumAndLargeDeviceSize = 12
          this.filterInpMedAndLargeDeviceSize = 12
          this.sortMedAndLargeDeviceSize = 12
      }
       ...
       generateColumnsArray(colArray){
            colArray.forEach(element=>{
                    ...
                    col.dataType  = element.DataType__c
                    col.isPickList = element.DataType__c=='picklist' ? true  : false,
                    col.options  = element.DataType__c=='picklist' ? this.getPicklistValues(element.FieldApi_Name__c)  : '',
                    col.isText = element.DataType__c!='picklist' && element.DataType__c!='generic' ? true  : false,
                    col.isGeneric = element.DataType__c=='generic' ? true  : false,
                    this.columnsData.push(col)
                    console.log('options is >> '+col.options)
                    console.log('options is >> '+JSON.stringify(col.options))
            })
       ...
       getPicklistValues(name){
        console.log('picklist values>> '+JSON.stringify(this.treeModel));
        console.log('picklist values>> '+JSON.stringify(this.treeModel[0]));
        let colArray = this.treeModel;
        /*colArray.forEach(element=>{
            if(element===name){
        ...
        for (let i = 0; i < colArray.length; i++) {
            console.log('col is '+colArray[i].label)
            console.log('col is '+colArray[i].items)
            console.log('name is '+name)
            if(colArray[i].label===name){
                return colArray[i].items;
            }
        }
                ...
                tableRows.push(
                     {
                         ...
                         isPickList:rowval[col.dataType]=='picklist' ? true  : false,
                         isText:rowval[col.dataType]=='text' ? true  : false,
                         isGeneric:rowval[col.dataType]=='generic' ? true  : false,
                         type : rowval[col.dataType] ? rowval[col.dataType]  : ' ',
                         Id : rowval.Id
                     })
                })
