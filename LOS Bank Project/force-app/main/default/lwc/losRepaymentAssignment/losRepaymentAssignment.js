...
import getApplicantInitData from '@salesforce/apex/RepaymentController.getApplicantInitData';
...
export default class LosRepaymentAssignment extends LightningElement {
    @api recordId
    @track loanRecordId = ''
    ...
    connectedCallback(){
        ...
        this.getApplicantInitData();
    }

    getApplicantInitData(){
        getApplicantInitData({
            assignmentId : this.recordId
        })
        .then(res=>{
            console.log('res '+JSON.stringify(res));
            this.loanRecordId = res.Loan_Application__c;
            this.setApplicantInitDataValues(res);
            // this.getDocRecords();
    ...
    setApplicantInitDataValues(data){
        this.loanApplicantionRecord = data.Loan_Application__r;
        this.countACHSI = this.loanApplicantionRecord.Count_of_ACH_SI__c!=undefined?this.loanApplicantionRecord.Count_of_ACH_SI__c:'';
        this.repaymentMode = this.loanApplicantionRecord.Repayment_Mode__c!=undefined?this.loanApplicantionRecord.Repayment_Mode__c:'';
        this.loanAmount = this.loanApplicantionRecord.Loan_Amount__c!=undefined?this.loanApplicantionRecord.Loan_Amount__c:'';
        this.loanTenure = this.loanApplicantionRecord.Tenure__c!=undefined?this.loanApplicantionRecord.Tenure__c:'';
        this.callbackStatus = this.loanApplicantionRecord.E_Mandate_Status__c!=undefined?this.loanApplicantionRecord.E_Mandate_Status__c:'';
        this.repaymentAccountNumber = this.loanApplicantionRecord.Repayment_Account_Number__c!=undefined?this.loanApplicantionRecord.Repayment_Account_Number__c:'';
        this.loanNumber = this.loanApplicantionRecord.Loan_Number__c != undefined?this.loanApplicantionRecord.Loan_Number__c:'';
        this.emiAmount = this.loanApplicantionRecord.EMI__c != undefined?this.loanApplicantionRecord.EMI__c:'';
        //this.emiDate = this.loanApplicantionRecord.EMI_Date__c != undefined?this.loanApplicantionRecord.EMI_Date__c:'';
        this.emiDate = this.loanApplicantionRecord.First_EMI_Date__c != undefined?this.loanApplicantionRecord.First_EMI_Date__c:'';
        
        //this.repaymentBankAccount = this.loanApplicantionRecord.Repayment_Mode__c!=undefined?this.loanApplicantionRecord.Repayment_Mode__c:'';
        this.repaymentBankAccount = this.loanApplicantionRecord.Repayment_Bank_Name__c!=undefined?this.loanApplicantionRecord.Repayment_Bank_Name__c:'';
        let val = this.loanApplicantionRecord.Emandate_Journey__c!=undefined?this.loanApplicantionRecord.Emandate_Journey__c:'';
        ...
        this.paymentType = this.loanApplicantionRecord.Emandate_Payment_Type__c!=undefined?this.loanApplicantionRecord.Emandate_Payment_Type__c:'';//getFieldValue(this.loanApplicantionRecord,Emandate_Payment_Type__c);
        ...
        if(this.repaymentMode.includes('NACH') || this.repaymentMode.includes('Standing Instructions')){
            this.isCountVisible = true;
            this.isSINACH = true;
            //this.showBankAccount = true;
            if(this.repaymentMode.includes('Standing Instructions')){
                this.iframeSrc = '/apex/RepaymentSIVFPage?recordid='+this.loanRecordId;
                this.showMobile = true;
                this.isSI = true;
                this.getCASADetails();
            }
            else{
                this.iframeSrc = '/apex/RepaymentACHVFPage?recordid='+this.loanRecordId;
                this.isGenerateDisabled = false;
            }
        }
        if(this.repaymentMode.includes('E mandate')){
            this.isEMandate = true;
            //this.showBankAccount = true;
        }
        if(this.repaymentMode.includes('NACH') || (this.repaymentMode.includes('E mandate') && this.loanAmount > 1000000)){
            this.type = 'SPDC';
        ...
        if(this.loanApplicantionRecord.SPDC_Details__c!=null){
            let temp = JSON.parse(this.loanApplicantionRecord.SPDC_Details__c).replaceAll("\\",'');
        ...
        if(this.loanApplicantionRecord.PDC_Details__c!=null){
            let temp = JSON.parse(this.loanApplicantionRecord.PDC_Details__c).replaceAll("\\",'');
            console.log('temp '+temp);
    ...
    getDocRecords(){
        this.isloading = true;
        getDocumentRecordsData({loanId : this.loanRecordId})
        .then((result) => {
                ...
                setTimeout(() => {
                    ...
                    parseResult.docChkRecords.forEach(element => {
                        if(element.docName.Document_Master__r.Document_Name__c == 'NACH'){
                            ...
                            this.spdc = element;
                            //alert(this.spdc.isUploded)
                            console.log('yash 1234'+JSON.stringify(this.spdc))
                            this.docDataReceived=true
                        }
                    });
    ...
    async submitClick(){
        if((this.repaymentMode.includes('NACH') || (this.repaymentMode.includes('E mandate') && this.loanAmount > 1000000)) && (!this.isReturnedFromPaymentFavouring)){
            // this.type = 'SPDC';
            // this.docName = 'AUWheels0079';
            // this.docTypeName = 'SPDC';
            // this.isWithoutpdc = true;
        ...
        restricAccess({
            compName: 'losRepaymentComponent' ,loanId: this.loanRecordId
            }).then(data => {
                if (data) {
    ...
    handleChange(event) {
        ...
        if(event.target.name=='Repayment Mode' && !(value.includes('NACH') || (this.repaymentMode.includes('E mandate') && this.loanAmount > 1000000))){
    ...
    generatePDF(){
        ...
        if(this.repaymentMode=='Standing Instructions'){
            uploadSIForm({loanId: this.loanRecordId}).then((result=>{
                this.isloading = false;
                let data;
                if(result!=null){
                    data = result.split(';');
            ...
            uploadACHForm({loanId: this.loanRecordId}).then((result=>{
                this.isloading = false;
                let data;
                if(result!=null){
                    data = result.split(';');
