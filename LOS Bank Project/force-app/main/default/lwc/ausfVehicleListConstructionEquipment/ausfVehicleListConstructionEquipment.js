...
import { getUniqueValue, OTHER_FUNDING_ITEMS_MAPPINGS, validateLoanFunding } from 'c/lwcutilities';
...
const VEHICLE_TYPE_VS_ALLOWED_DELETE_STAGES = {
    ...
    'Construction Equipment' : [ 'QDE', 'DDE', 'Credit' ] //R2-2787
};
...
export default class AusfVehicleListConstructionEquipment extends NavigationMixin(LightningElement) {
    label = {
    ...
    totalloanAmount;
    mdetailsOptions=[];
    companyofLoanSurakshaOptions=[];
    productofLoanSurakshaOptions=[];
    spinnerImage; 
    isRegistrable;
    loanApplication;
            ...
            .then(data => {
                if (data) {
                    ...
                    this._vehicleRecord = { ...data.coll };
                     //R2 || START
                    this.handleFilterOptions(customObject.Attachment__c);
                    this.isAttachment = customObject.Attachment__c == 'Yes' ? true : false;
                    this.newVehicleRecord.Attachment__c = customObject.Attachment__c;
                    this.makeOptions.push({label :customObject.Make__c ,value :customObject.Make__c });
                    this.makeOptionValue =customObject.Make__c;
                    this.isRegistrable = customObject.Registrable__c == 'Yes' ? true : false;
                    //this.handleMakeOption(customObject.Collateral_Name__c);
                    //R2 || END 
    ...
    async handleMakeOption(val){
        ...
        result.forEach(dataInstance => {
         ...
         this.makeOptions = getUniqueValue(options);
        // this.makeOptionValue = this.newVehicleRecord.Make__c;
                ...
                if (data) {
                 ...
                 if (collatList.length > 0) {
                        if (data.collateralList[0].Collateral_ID__c) {
                            ...
                            if(this.applicantLst.length>0 && this.applicantLst!=null){
                            const Obj = {};
    ...
    getLtvRecord() {
        this.isLoaded = true;
        /*const result = await getLtvMasterRecord({ loanApp: this.loanApplicationRecord, mapOfColl: JSON.stringify(this.newVehicleRecord) })
        .catch(err => {
            console.error(err);
        ...
        */
        ...
        if ( result ){
            ...
            if ( result.Assessment_Method__c === 'CE' ) {
                ...
                //this.newVehicleRecord = { ...this.newVehicleRecord, Assessment_Method__c: 'LTV', Approved_LTV__c: result[ `Commercial_${this.loanApplicationRecord.Customer_Grade__c}__c` ] };
               
                    let ltvObj = result;
                    this.newVehicleRecord.Assessment_Method__c = 'LTV';
                    if (this.loanApplicationRecord.Customer_Grade__c === 'FTB') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_FTB__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'FTU') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_FTU__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Hirer'  && this.loanApplicationRecord.Sub_Grade__c =='Small') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Plant_Hirer_Small__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Hirer'  && this.loanApplicationRecord.Sub_Grade__c =='Medium') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Plant_Hirer_Medium__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Hirer'  && this.loanApplicationRecord.Sub_Grade__c =='Large') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Plant_Hirer_Large__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Mine Owner'  && this.loanApplicationRecord.Sub_Grade__c =='Small-1') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Mine_owner_Small__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Mine Owner'  && this.loanApplicationRecord.Sub_Grade__c =='Medium-2-5') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Mine_owner_Medium__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Mine Owner'  && this.loanApplicationRecord.Sub_Grade__c =='Large-6+') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Mine_owner_Large__c	;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Contractor'  && this.loanApplicationRecord.Sub_Grade__c =='Small-1') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Contractor_Small__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Contractor'  && this.loanApplicationRecord.Sub_Grade__c =='Medium-2-5') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Contractor_Medium__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Contractor'  && this.loanApplicationRecord.Sub_Grade__c =='Large-6+') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Contractor_Large__c	;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Captive'  && this.loanApplicationRecord.Sub_Grade__c =='With financials') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Captive_With_Financial__c;
                    }
                    if (this.loanApplicationRecord.Customer_Grade__c === 'Captive'  && this.loanApplicationRecord.Sub_Grade__c =='Without financials') {
                        this.newVehicleRecord.Approved_LTV__c = ltvObj.CE_Captive_Without_Financial__c	;
                    }
                ...
                const absoluteAmount = ( +result[ `Commercial_${this.loanApplicationRecord.Customer_Grade__c}__c` ] || 0 );
    ...
    handleMakeValueChange(event) {
        this.handleMakeChange(event.target.value);
    }

    handleMakeChange(val){
        this.makeOptionValue = val;
        this.desableField.Model__c = false;
        this.variantOptionValue =null; //R2
    ...
    handleValueChange(event) {
        ...
        if (event.target.label === 'manufactureYear') {
            ...
            // this.netOfDiscountPrice(); //updated with getter function - dry run issue - Cost not getting calculated after input of Ex-Showroom
        } else if (event.target.name === 'LS_Value__c') {
            // this.vehicleCost(); //updated with getter function - dry run issue - Cost not getting calculated after input of Ex-Showroom
        }
    ...
    /*netOfDiscountPrice() {
        const collateral = { ...this.newVehicleRecord },
            exShowroomPrice = collateral.Ex_Showroom_Price__c,
            discount = collateral.Discount__c;

        let finalValue = 0;
        if( ( parseFloat(exShowroomPrice) || parseFloat(discount) ) && parseFloat(exShowroomPrice) >= parseFloat(discount) ){
            finalValue = parseFloat(exShowroomPrice) - parseFloat(discount);
        }
    ...
    get vehicleCost(){
        ...
        let discountPrice = +this.newVehicleRecord.Net_of_Discount_Price__c || 0;
        let lsAmount = +this.newVehicleRecord.LS_Value__c || 0;
        const vehicleCost = discountPrice + lsAmount;
        if( this.newVehicleRecord.Vehicle_Cost__c !== vehicleCost ){
            this.newVehicleRecord.Vehicle_Cost__c = vehicleCost ;
        }
    ...
    wiredValues({ error, data }) {
        if (data) {
            ...
            this.bodyTypeOptions = data.picklistFieldValues.Body_Type__c.values;
    ...
    async handleFilterOptions(val){
        ...
        //console.log('collateralPicklist-->' +JSON.stringify(this.collateralPicklist));
