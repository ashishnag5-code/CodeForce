...
import LOAN_FIELD from '@salesforce/schema/Assignment__c.Loan_Application__c';
...
export default class DiscrepancyButtonComponent extends LightningElement {
    ...
    @wire(getRecord, { recordId: '$recordId', fields: [ LOAN_FIELD ] })
    assignment;

    @wire(getRecord, { recordId: '$loanId', fields: [ OPS_DISCREPANCY_REMARKS_FIELD, RO_DISCREPANCY_REMARKS_FIELD, DISCREPANCY_COUNT_FIELD ] })
    loanApp;

    get loanId(){
        return getFieldValue( this.assignment?.data, LOAN_FIELD );
    }
    get opsRemarks(){
        return this.remarks || getFieldValue( this.loanApp?.data, OPS_DISCREPANCY_REMARKS_FIELD );
    }

    // SFAU-4293 - RO remarks not showing for OPS
    get roRemarks(){
        return getFieldValue( this.loanApp?.data, RO_DISCREPANCY_REMARKS_FIELD );
    }
    get showRORemarks(){
    ...
    handleSubmit() {
        ...
        this.isLoading = true
        if(this.remarks) { 
            updateLoanApplicationRec({ recordId : this.recordId, remarks : this.remarks, opsStatus : 'Discrepancy' })
            .then((result) => {
