...
import checkIfLoanSourcedInSF from '@salesforce/apex/AUSFVehicleController.checkIfLoanSourcedInSF'; 
...
import getDepreciatedMasterRecord from '@salesforce/apex/AUSFCommercialVehicleController.getDepreciatedMaster';
...
import vahanNonResgistrableAsset from '@salesforce/label/c.VahanNonRegistrableAsset';
import createValuationDeviation from '@salesforce/apex/AUSFVehicleController.createValuationDeviation';
import S4GridMinDiffPercentage from '@salesforce/label/c.S4GridMinCommercialDiffPercentage';
import S4GridMaxDiffPercentage from '@salesforce/label/c.S4GridMaxCommercialDiffPercentage';
import S5GridMaxDiffPercentage from '@salesforce/label/c.S5GridMaxCommercialDiffPercentage';
import getCollateralNames from  '@salesforce/apex/AUSFVehicleCEController.getCollateralNames';
import getCEMakeNames from '@salesforce/apex/AUSFVehicleCEController.getCEMakeNames';
import getApplicants from '@salesforce/apex/financeController.getApplicants';
import { getSpinnerImage } from 'c/customSpinner';
import { getLeastAmount, OTHER_FUNDING_ITEMS_MAPPINGS, validateLoanFunding } from 'c/lwcutilities';
import updateLoanApp from '@salesforce/apex/FinancialViewTemplateR2Controller.updateLoanApp';
import getAttachmentFundingCapAmount from '@salesforce/apex/AUSFVehicleCEController.getAttachmentFundingCapAmount';
...
const NON_REGISTRABLE_ASSET_IDENTIFIER = 'NONREGISTRABLE';
...
const VEHICLE_TYPE_VS_ALLOWED_DELETE_STAGES = {
    'Construction Equipment': [ 'QDE', 'DDE', 'Credit' ],
};
...
export default class AusfVehicleListUsedConstructionEquipment extends NavigationMixin(LightningElement) {
    @api recordId;
    ...
    isLanCreated;
    isTractor;
    dipreciatedMasterRecord;
    applicantFinancialsRecord;
    newDeviationRecord={};
    attachmentOptions=[];
    regisOptions=[];
    collateralPicklist=[];
    collateralFilteredPicklist =[];
    repoOptions=[];
    ...
    */
    ...
    get productSubTypeOptions() {
        return [
            ...
            { label: 'Tractor + Loader', value: 'Tractor + Loader' },
            { label: 'Tractor + Dozer', value: 'Tractor + Dozer' },
            { label: 'Tractor + Loader & Doze', value: 'Tractor + Loader & Doze' },
            { label: 'Tractor + new implement', value: 'Tractor + new implement' },
        ];
    }
    ...
    get isSameAsVehicleOwner(){
        ...
        //SFAU-5228 : Added by Samridhi | this.productName && (this.productName.toUpperCase().includes('CASH ON WHEELS') || this.productName.toUpperCase().includes('USED')) && - Used only for use/cow cases
        let isSameOwner = '';
        if(this.newVehicleRecord.Loan__c && this.newVehicleRecord.HPN_With_Financiers_Name__c && this.newVehicleRecord.HPN_With_Financiers_Name__c.toUpperCase().startsWith('AU',0)){
    ...
    label = {
        ...
        S4GridMinDiffPercentage,
        S4GridMaxDiffPercentage,
        S5GridMaxDiffPercentage
    };
    ...
    fetchState(loanApplRecordId) {
		...
		.then(async data => {
            if (data) {
                ...
                this.newVehicleRecord.Total_Loan_Amount__c = this.loanApplicationRecord.Loan_Amount__c;
                ...
                this.collateralPicklist = data.collateralNames.Collateral_Name__c;
                ...
                this.totalloanAmount = loanApplication.Total_Loan_Amount__c;
    ...
    getVehicleDetail(recordVehicleId){
		...
		.then(async data => {
            if (data) {
                console.log('tractorUsedSavedDate'+JSON.stringify(data))
                ...
                this.newVehicleRecord = customObject;
                 //R2 || START
                    this.handleFilterOptions( this.newVehicleRecord.Attachment__c);
                    this.isAttachment = this.newVehicleRecord.Attachment__c == 'Yes' ? true : false;
                    this.newVehicleRecord.Attachment__c = this.newVehicleRecord.Attachment__c;
                    let options=[];
                    options.push({label :customObject.Make__c ,value :customObject.Make__c });
                    this.makeOptions = options;
                    this.makeOptionValue = this.newVehicleRecord.Make__c;
                    this.isRegistrable = this.newVehicleRecord.Registrable__c == 'Yes' ? true : false;
                   // this.handleMakeOption( this.newVehicleRecord.Collateral_Name__c);
                    //R2 || END 
    ...
    handleCopyToCollateral(event){
        ...
        this.newVehicleRecord.LS__c = true;
        this.newVehicleRecord.Insurance_Funding__c = true;
        ...
        if(!this.newVehicleRecord.POS__c){
            ...
            if(posValue >0){
                checkIfLoanSourcedInSF({collateralId : currentObj.Collateral_ID__c})
                .then(result => {
                    if (result) {
                        if(result.Loan__r.Stage__c != 'PDD'){
    ...
    getMmvRecord(variantId){
        ...
        .then(result => {
            ...
            if(result.Id){
                ...
                this.newVehicleRecord.HP__c = result.HP__c;
    ...
    handleCollateralName(event) {
        ...
        this.handleMakeOption(event.target.value); //R2
    }
    //R2
    async handleMakeOption(val){
        this.makeOptionValue = null;
        let result = await getCEMakeNames({ collateralId :val, attachmentValue : this.newVehicleRecord.Attachment__c});
        let options=[];
        result.forEach(dataInstance => {
            options.push({label:dataInstance,value:dataInstance})
         });
         this.makeOptions = options;
        // this.makeOptionValue = this.newVehicleRecord.Make__c;
    }
    ...
    handleVahaanData(){
        ...
        if( vehicleNumber === NON_REGISTRABLE_ASSET_IDENTIFIER ){
            this.showToast( vahanNonResgistrableAsset, 'error');
            return;
        }
        this.getVahaanDetail(vehicleNumber, this.recordId);
    }
		...
		.then(data => {
            if (data) { 
                ...
                if(isValidLoanAmount && isValidTenure && isValidRoi){
                    this.getDepreciatedObj( {...this.newVehicleRecord}, {...this.loanApplicationRecord} )
                }else{
    ...
    getDepreciatedObj( collateral, loanApp ){
        this.isLoaded = true;

        getDepreciatedMasterRecord({ collateral, loanApp })
		.then(data => {
            if (data) { 
                console.log('getDepreciatedObj >>'+JSON.stringify(data))
                this.depreciatedObjRecord = data;
                this.newVehicleRecord = { ...this.newVehicleRecord, LTV_Approved__c: JSON.stringify( data ), Approved_LTV__c: data.LTV_Approved__c, Assessment_Method__c: 'LTV' };
                this.isLoaded = false;
            }else{
                this.isLoaded = false;
            }
    ...
    handleAdditionalInformationClick(){
        ...
        curObj.Insurance_Funding__c = true;
    ...
    handleValueChange(event){
        ...
        if( fieldApi == 'Valuation_Amount__c' || fieldApi == 'Approved_LTV__c'){
            this.handlevehicleCost();
        }
        if( fieldApi == 'Registrable__c'){
             
            this.isRegistrable = fieldValue == 'No' ? false : true;
            this.isVahaanCheckIsMendatory = this.isRegistrable || this.disabledFetchVahhan; //R2-2693 - If Vahan already called by considering as registrable asset, updating Registrable to No & back to Yes consider the last Vahan check
        }
    ...
    handlevehicleCost(){
        ...
        // grid value or valuation amount which ever is lower 
        let approvedLTV = this.newVehicleRecord.Approved_LTV__c!=null && this.newVehicleRecord.Approved_LTV__c!='' && this.newVehicleRecord.Approved_LTV__c!=NaN  && this.newVehicleRecord.Approved_LTV__c!=undefined ? parseFloat(this.newVehicleRecord.Approved_LTV__c):0;
        let valuationAmt = this.newVehicleRecord.Valuation_Amount__c!=null && this.newVehicleRecord.Valuation_Amount__c!='' && this.newVehicleRecord.Valuation_Amount__c!=NaN  && this.newVehicleRecord.Valuation_Amount__c!=undefined? parseFloat(this.newVehicleRecord.Valuation_Amount__c) :0;
        this.newVehicleRecord.Vehicle_Cost__c =  approvedLTV!=undefined && approvedLTV!=0 ? approvedLTV :0;
        if(approvedLTV == 0 && valuationAmt!=0){
             this.newVehicleRecord.Vehicle_Cost__c = valuationAmt;
        }else if( valuationAmt == 0 && approvedLTV!=0){
             this.newVehicleRecord.Vehicle_Cost__c = approvedLTV;
        }else{
            this.newVehicleRecord.Vehicle_Cost__c = approvedLTV < valuationAmt ? approvedLTV : valuationAmt;
        }
        //Depriciated master (on grid or valuation which ever is lower)
        //this.newVehicleRecord.Approved_LTV__c =  approvedLTV;
    ...
    handleSubmitForm(){
        restricAccess({
            ...
            .then(data => {
                ...
                let currentObj = Object.assign({}, this.newVehicleRecord);
                let insuranceAmount = currentObj.Insurance_Value__c? parseFloat(currentObj.Insurance_Value__c):0;
                let lsAmount = currentObj.LS_Value__c?parseFloat(currentObj.LS_Value__c):0;
                let valuationAmt = currentObj.Valuation_Amount__c?parseFloat(currentObj.Valuation_Amount__c):0;
                let totalFunding = insuranceAmount + lsAmount;
                if(totalFunding > valuationAmt){
                    this.showToast( 'Funding cannot be more than Valuation Amount','error' );
                    currentObj.LS_Value__c='';
                    currentObj.Valuation_Amount__c='';
                    currentObj.Insurance_Value__c ='';
                    return;
                }
                if(valuationAmt && currentObj.Apportioned_Loan_Amount__c > valuationAmt){
                   this.showToast( 'Loan Amount cannot be more than Valuation Amount','error' );
                    return;
                }
    ...
    async upsertVehicleInfo() {
        ...
        currentObj.Total_Loan_Amount__c = this.totalLoanAmount;
        
        if( currentObj.Capping_Amount__c && +currentObj.Apportioned_Loan_Amount__c > currentObj.Capping_Amount__c ){
        ...
        this.calculateLoanAmtGridDifference();
    }
    ...
    get ltvOfferedOnVehicleFinalCost(){
        ...
        // let loanAmount = this.newVehicleRecord.Apportioned_Loan_Amount__c ? this.newVehicleRecord.Apportioned_Loan_Amount__c : 0;
        const totalLoanAmount = this._totalLoanAmount;//this.newVehicleRecord.Loan__r?.Total_Loan_Amount__c ?? this.loanApplicationRecord?.Total_Loan_Amount__c;
        if(this.isTwoWheeler){
    ...
    get loanEligibilty(){
        ...
        //parseInt Converts a string to an integer.
            ...
            if(parseInt(approvedLtvAmount,10) && parseInt(loanAmount,10)!==0){
                ...
                finalValue = (parseInt(approvedLtvAmount,10)/100) * parseInt(loanAmount,10)
                //this.finalDisValue = finalValue;
    ...
    get finalPrice(){
        ...
        const { Valuation_Amount__c: valuationAmount, Certified_Value__c: certifiedValue, IBB_Value__c: ibbValue, Grid_Depreciated_Value__c: depreciatedValue } = currentObj || {};
        const certifiedValueNum = +certifiedValue?.replace(/,/g, ''), valuationAmountNum = +valuationAmount?.replace(/,/g, ''), depreciatedValueNum = +depreciatedValue?.replace(/,/g, '');

        const applicableCost = getLeastAmount( vehicleCost, [ depreciatedValueNum, certifiedValueNum || valuationAmountNum ] ) || 0;
        console.log({ depreciatedValueNum, applicableCost, valuationAmountNum });
    ...
    get vehicleCost(){
        ...
        const { Vehicle_Cost__c: vehicleCost, IBB_Value__c: ibbValue, Valuation_Amount__c: valuationAmount, Certified_Value__c: certifiedValue, Grid_Depreciated_Value__c: depreciatedValue } = this.newVehicleRecord || {};
        const valuationAmountNum = +valuationAmount?.replace(/,/g, ''), certifiedValueNum = +certifiedValue?.replace(/,/g, ''), depreciatedValueNum = +depreciatedValue?.replace(/,/g, '');
    ...
    getDLPFMPMaster(){
        ...
        Promise.all([
            ...
            getDepreciatedMaster({ loanApp: this.loanApplicationRecord, collateral: this.newVehicleRecord }),
            
        ]).then((values) => {
            console.log('gridValues-->' +JSON.stringify(values));
            if(values[0] != undefined){   
                this.applicantFinancialsRecord = values[0];
            }
            if(values[1] != undefined){
                const depreciatedMaster = values[1];
                if( depreciatedMaster && Object.keys(depreciatedMaster).length ){
                    this.newVehicleRecord = {
                        ...this.newVehicleRecord,
                        LTV_Record__c: JSON.stringify(depreciatedMaster),
                        Approved_LTV__c: depreciatedMaster.LTV_Approved__c,
                        Assessment_Method__c: 'LTV'
                    };
                }
            ...
            if (data) {
                ...
                console.log('PicklistValues Category-->' +JSON.stringify(data.Category));
                    ...
                    if(data.Registrable?.length){
                        ...
                        if(registrable){
                            this.newVehicleRecord.Registrable__c = registrable.value;
                            this.isRegistrable = registrable.value == 'Yes' ? true : false;
                        }
                    }
    ...
    wiredValues({ error, data }) {
        if (data) {
            ...
            let options=data.picklistFieldValues.Engine_Category__c.values;
            const filteredEquipment = options.filter(item => item.value!='BS2' && item.value!='BS3');
            this.engineCategoryOptions =filteredEquipment;
            ...
            this.attachmentOptions = data.picklistFieldValues.Attachment__c.values;
            this.regisOptions = data.picklistFieldValues.Registrable__c.values;
            this.repoOptions = data.picklistFieldValues.Repo_Sale_Vehicle_CBS__c.values;
            this.attachmentTypeOptions = data.picklistFieldValues.Attachment_Type__c.values; 
    ...
    get _totalLoanAmount(){
        return (+this.newVehicleRecord.Apportioned_Loan_Amount__c || 0) + this.otherFundingTotal;
    }
    ...
    calculateLoanAmtGridDifference(){
        
        let gridVal = parseFloat(this.newVehicleRecord.Grid_Depreciated_Value__c);
        let loanAmt = parseFloat(this.totalloanAmount);
        if(loanAmt > gridVal){
        let differencePercentage = (( loanAmt - gridVal)/gridVal) * 100;
        let deviationLevel ='';
        let deviationMsg ='';
        let S4GridMinDiffPercentage = parseFloat(this.label.S4GridMinDiffPercentage);
        let S4GridMaxDiffPercentage = parseFloat(this.label.S4GridMaxDiffPercentage);
        let S5GridMaxDiffPercentage = parseFloat(this.label.S5GridMaxDiffPercentage);
        
        if( differencePercentage >= S4GridMinDiffPercentage && S4GridMaxDiffPercentage <= 120 ){
            deviationMsg = 'S4 level approval will be required';
            this.showToast(deviationMsg,'warning');
            deviationLevel = 'S4';
        }else if( differencePercentage > S5GridMaxDiffPercentage ){
            deviationMsg = 'S5 level approval will be required';
            this.showToast(deviationMsg,'warning');
            deviationLevel = 'S5';
        }

        this.newDeviationRecord.Loan_Application__c = this.recordId;
        this.newDeviationRecord.Level__c = deviationLevel;
        this.newDeviationRecord.Active__c = true;
        this.newDeviationRecord.Deviation_Type__c = deviationLevel+'-'+'Valuation';
        this.newDeviationRecord.Source__c = 'SF';
        this.newDeviationRecord.Archived__c = false;
        this.newDeviationRecord.Department__c = 'Credit'; //R2-2617
        this.newDeviationRecord.Details__c = deviationMsg;
        let operationMsg = deviationMsg!='' ?'New':'Archive'; 
        createValuationDeviation({ deviationRecord : this.newDeviationRecord,loanId:this.recordId , operation : operationMsg })
        .then(result => {
           console.log('deviationCreated');
        })
		.catch(error => {
            console.log('error is '+JSON.stringify(error));
            this.isLoaded = false;
    ...
    handleAttachmentChange(event){
        ...
        this.handleFilterOptions(value);
        this.newVehicleRecord.Attachment__c = value;
        this.isAttachment =  value == 'Yes' ? true : false; 
    }
    async handleAttachmentTypeChange(event){
        let attachmentType = event.target.value;
        this.newVehicleRecord.Attachment_Type__c = attachmentType;
        let loanId = this.recordId;

        const capMaster = await getAttachmentFundingCapAmount({ loanId ,attachmentType })
            .catch(err => {
                console.error( err );
    ...
    async handleFilterOptions(val){
        this.newVehicleRecord.Collateral_Name__c =null;
        let filteredCollateralValues = await getCollateralNames({ attachment :val });
        const equipmentList = this.collateralPicklist;
        const filteredEquipment = equipmentList.filter(item => filteredCollateralValues.includes(item.value));
        console.log('filteredEquipment-->' +JSON.stringify(filteredEquipment));
        this.collateralFilteredPicklist = filteredEquipment;
        
    }
     updateLoanApp(){
          console.log('this.recordId-->' +this.recordId);
         // this.loanApplication.Id = this.recordId;
          //this.loanApplication.Collateral_Type__c = this.newVehicleRecord.Collateral_Name__c;
          this.loanApplicationRecord.Collateral_Type__c = this.newVehicleRecord.Collateral_Name__c;
        updateLoanApp({loan: this.loanApplicationRecord}).then((data=>{
            if(data=='Success'){
               console.log('updatedLoanCollateralType');
